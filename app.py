...
def main():
    ...
    if page == "Dashboard":
        ...
        initial, posted, pending, total = get_dashboard_summary()

        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Initial FY Balance", f"${initial:,.2f}")
        col2.metric("Latest Posted Balance", f"${posted:,.2f}")
        col3.metric("Pending Transactions", f"${pending:,.2f}")
        col4.metric("Projected Total", f"${total:,.2f}")
